 ...
 * @author Neil Ellis
 */
class RegexResource extends AbstractResource {

    RegexResource(EinsteinURI uri) {
        super(uri);
        Pattern pattern = Pattern.compile(uri.getDescriptor().asString());

        addMapping(QueryFacade.class, new RegexQueryFacade(pattern));
        addMapping(BooleanQueryFacade.class, new RegexQueryFacade(pattern));
    }
