 ...
 * @author Neil Ellis
 */
@RegisterInstruction(name = "merge",
                     metadata = @InstructionMetaData(
                             core = @CoreMetaData(
                                     name = "merge",
                                     shortDescription = "Merge multiple element tuples into a single one.",
                                     description = "Merges multiple messages into a single one, converts a multiple message tuple into a single message tuple..",
                                     syntax = "( 'merge'  | ( '(' '~' ')' ) | (  '~' <instruction-group> ) | ( <instruction-group> ( '~' <instruction-group> ) + ) )",
                                     example = "~ [ read \"text:A\",[ read \"text:B\", read \"text:C\" ] ] ;"
                             ),
                             result = "A single item tuple"
                     ))

@RegisterOperator(name = "merge",
                  symbol = "~",
                  types = {OperatorType.UNARY_REVERSED, OperatorType.OPERATOR_ONLY, OperatorType.N_ARY})
@org.contract4j5.contract.Contract
public class MergeInstruction extends AbstractInstruction {

    public MergeInstruction(CompilationInformation debug) {
        super(debug);
    }
    ...
    public MessageTuple execute(ExecutionContext context, MessageTuple message) {
        ...
        return getParent().getActiveProfile()
                .getMessageModel()
                .merge(message, context.getActiveProfile().getDataModel());
    }
}
