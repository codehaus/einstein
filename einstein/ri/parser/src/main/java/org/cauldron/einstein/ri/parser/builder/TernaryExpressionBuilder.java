...
import org.cauldron.einstein.ri.parser.tree.TernaryNode;
...
public class TernaryExpressionBuilder extends AbstractEinsteinNodeBuilder {
    ...
    public EinsteinNode build(EinsteinParseVisitor visitor, ASTNode astNode, EinsteinNode parent) {
        ...
        if (astNode.getNumberOfChildren() == 4) {
            // (this) ? "that" : {something}
            operatorSymbol = astNode.getSecondChild().toString().trim();
        } else {
            // ? "that" : {something}
            operatorSymbol = astNode.getFirstChild().toString().trim();
        ...
        if (operatorTypes.contains(OperatorType.TERNARY)) {
            TernaryNode node = new TernaryNode(astNode, operatorSymbol, operator);
            parent.addChild(node);
            visitor.yieldToChildren(astNode, node);
            return node;
        } else {
            throw new EinsteinParseException(BUNDLE_NAME, astNode, OPERATOR_NOT_TERNARY, operatorSymbol);
        }
