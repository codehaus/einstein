...
import org.cauldron.einstein.ri.parser.tree.UnaryNode;
...
public class UnaryExpressionBuilder extends AbstractEinsteinNodeBuilder {
    ...
    public EinsteinNode build(EinsteinParseVisitor visitor, ASTNode astNode, EinsteinNode parent) {
        ...
        if (!operatorTypes.contains(OperatorType.UNARY) && !operatorTypes.contains(OperatorType.UNARY_REVERSED)) {
            throw new EinsteinParseException(BUNDLE_NAME, astNode, OPERATOR_NOT_UNARY, operatorSymbol);
        }

        UnaryNode unaryNode = new UnaryNode(astNode, operator);
        parent.addChild(unaryNode);
        visitor.yieldToChildren(astNode, unaryNode);
        return unaryNode;
