 ...
 * @author Neil Ellis
 */
@RegisterProvider(name = "console",
        metadata = @ProviderMetaData(
                core = @CoreMetaData(
                        name = "console",
                        shortDescription = "Provides access to console input and console output.",
                        description = "Provides access to console input and console output.",
                        syntax = "'console:' <free-format-prefix>",
                        example = ">> \"console:This is the current message's payload as a string:\";"
                ),
                alwaysSupported = {ReadFacade.class, WriteFacade.class, PollFacade.class}


        )
)
@org.contract4j5.contract.Contract
public class ConsoleProvider extends AbstractProvider {


    public Resource getLocalResource(EinsteinURI uri, Class[] facades) {
        return new ConsoleResource(uri);
    }
}
