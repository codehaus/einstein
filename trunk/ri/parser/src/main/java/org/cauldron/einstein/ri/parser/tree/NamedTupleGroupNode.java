...
import org.cauldron.einstein.ri.core.group.NamedTupleInstructionGroupImpl;
...
public class NamedTupleGroupNode extends AbstractEinsteinNode implements ExpressionNode, GroupNode {
    private final List<LabelledExpressionNode> expressions = new ArrayList<LabelledExpressionNode>();

    public NamedTupleGroupNode(ASTNode astNode) {
        super(astNode);
    }
    ...
    public COMNode toCOMNode(COMNode parent) {
        ...
        constructorCall = new ConstructorCall(astNode, new CompiledClass(NamedTupleInstructionGroupImpl.class));
        constructorCall.insert(DebugInfoHelper.getDebugCOMNodeFor(astNode));
        parent.insert(primaryExpression).insert(constructorCall);
        for (LabelledExpressionNode labelledExpressionNode : expressions) {
            labelledExpressionNode.toCOMNode(constructorCall);
        }
    ...
    public void addChildInternal(LabelledExpressionNode node) {
